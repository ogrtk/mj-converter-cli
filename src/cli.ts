...
function loadConfig(configPath: string): AppConfig {
  ...
  try {
    ...
    if (!Array.isArray(config.conversion.targetColumns) || config.conversion.targetColumns.length === 0) {
    ...
    // ヘッダーの整合性チェック
    if (!config.conversion.input.hasHeader && config.conversion.output.hasHeader) {
      throw new Error('入力ファイルにヘッダーがないのに、出力ファイルにヘッダーを出力することはできません');
    }
...
export async function main(): Promise<number> {
  try {
    ...
    if (!fs.existsSync(config.conversion.input.path)) {
      logger.error(`入力ファイルが見つかりません: ${config.conversion.input.path}`);
      return 1;
    }
    
    // 文字変換表の存在チェック
    if (!fs.existsSync(config.conversion.conversionTable)) {
      logger.error(`文字変換表ファイルが見つかりません: ${config.conversion.conversionTable}`);
      return 1;
    }
    
    // 出力ディレクトリの作成
    const outputDir = config.conversion.output.path.substring(0, config.conversion.output.path.lastIndexOf('/'));
    ...
    if (result.success) {
      logger.info(`処理が正常に完了しました。処理行数: ${result.processedRows}行`);
      if (result.hasWarnings) {
        logger.warn('処理中に警告が発生しました。詳細はログファイルを確認してください。');
        return 2; // 警告終了コード
      }
      return 0; // 正常終了
    } else {
