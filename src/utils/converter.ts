...
import { splitCharacters, joinCharacters } from './unicode.js';
 ...
 */
export async function loadConversionTable(filePath: string): Promise<ConversionMap> {
  ...
  try {
    ...
    for (let i = 0; i < records.length; i++) {
      ...
      if (!record || record.length < 2) {
        logger.warn(`行${i + 1}: 変換表の形式が正しくありません (列数: ${record?.length || 0})`);
        errorCount++;
        continue;
      }
    ...
    logger.info(`文字変換表読み込み完了: 有効${validCount}件, エラー${errorCount}件`);
    ...
    logger.error(`文字変換表読み込みエラー: ${error instanceof Error ? error.message : String(error)}`);
    throw error;
  }
}
 ...
 * 文字列を変換表に基づいて1文字ずつ変換
 */
export function convertString(text: string, conversionMap: ConversionMap): string {
  ...
  for (const char of chars) {
    ...
    if (converted !== undefined) {
      ...
      switch (missingCharacterHandling) {
        ...
        case 'warn':
          logger.warn(`【文字変換警告】文字 "${char}" (U+${char.codePointAt(0)?.toString(16).padStart(4, '0')}) が変換表に見つかりません。そのまま出力します。`);
          convertedChars.push(char);
          hasWarnings = true;
          break;
        case 'skip':
...
export function convertCsvRecord(
  ...
  for (const columnIndex of targetColumns) {
    if (columnIndex >= 0 && columnIndex < record.length) {
      ...
      const result = convertStringWithHandling(originalValue, conversionMap, missingCharacterHandling);
      ...
      if (originalValue !== result.convertedValue) {
        logger.debug(`列${columnIndex + 1}: "${originalValue}" → "${result.convertedValue}"`);
      }
    } else {
      logger.warn(`列インデックス ${columnIndex} は範囲外です (最大: ${record.length - 1})`);
      hasWarnings = true;
    }
  }
