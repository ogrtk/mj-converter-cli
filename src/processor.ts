...
import { loadConversionTable, convertCsvRecord, type ConversionMap } from './utils/converter.js';
 ...
 * CSV文字変換の主要処理
 */
export async function processConversion(config: ConversionConfig): Promise<ProcessResult> {
  ...
  try {
    ...
    logger.info(`入力ファイルヘッダー: ${config.input.hasHeader ? 'あり' : 'なし'}`);
    logger.info(`出力ファイルヘッダー: ${config.output.hasHeader ? 'あり' : 'なし'}`);
    logger.info(`文字変換表にない文字の処理方式: ${config.missingCharacterHandling}`);
    logger.info('-'.repeat(60));
    ...
    for (let i = 0; i < Math.min(inputRecords.length, 5); i++) {
      ...
      if (record && record.length < expectedColumns) {
        logger.warn(`【警告】行${i + 1}: 列数不足 (実際:${record.length}, 必要:${expectedColumns})`);
      }
    }
    ...
    if (config.input.hasHeader && inputRecords.length > 0) {
      ...
      startRowIndex = 1;
      logger.info(`入力ファイルのヘッダー行を検出: [${headerRecord?.join(', ')}]`);
    } else {
    ...
    if (config.output.hasHeader) {
      if (headerRecord) {
        ...
        logger.warn('【警告】出力にヘッダーが必要ですが、入力ファイルにヘッダーがありません');
        hasWarnings = true;
      }
    } else {
    ...
    logger.info(`処理対象データ行数: ${dataRowCount}行 (${startRowIndex + 1}行目～${inputRecords.length}行目)`);
    logger.info('-'.repeat(60));
    ...
    for (let i = startRowIndex; i < inputRecords.length; i++) {
      ...
      try {
        const result = convertCsvRecord(record, config.targetColumns, conversionMap, config.missingCharacterHandling);
        ...
        if (config.missingCharacterHandling === 'error') {
          logger.error(`行${i + 1}の処理でエラー: ${error instanceof Error ? error.message : String(error)}`);
          throw error;
        } else {
          logger.error(`行${i + 1}の処理でエラー: ${error instanceof Error ? error.message : String(error)}`);
          // エラーが発生した行もそのまま出力
    ...
    logger.info(`入力ファイル: ${path.resolve(config.input.path)} (${inputRecords.length}行)`);
    logger.info(`出力ファイル: ${path.resolve(config.output.path)} (${outputRecords.length}行)`);
    ...
    if (hasWarnings) {
      logger.warn('⚠️  処理中に警告が発生しました。上記ログの詳細を確認してください。');
    } else {
